...
class UserPreferencesRepository @Inject constructor(@ApplicationContext context: Context) {
    ...
    val themeMode: Flow<String> =
        dataStore.data.map { preferences -> preferences[PreferencesKeys.THEME_MODE] ?: "SYSTEM" }

    val useDynamicColor: Flow<Boolean> =
        dataStore.data.map { preferences -> preferences[PreferencesKeys.USE_DYNAMIC_COLOR] ?: true }
    ...
    suspend fun setThemeMode(mode: String) {
        dataStore.edit { preferences -> preferences[PreferencesKeys.THEME_MODE] = mode }
    }
