...
data class PlateCount(val weight: Double, val count: Int)

class PlateCalculator @Inject constructor() {
    @Suppress("NestedBlockDepth")
    fun parse(input: String): List<PlateCount> {
        val tokens =
            input.replace("+", " ").replace("*", "x").replace("×", "x").split(" ").filter {
                it.isNotBlank()
            }
    ...
    fun calculateTotalWeight(input: String, barWeight: Double = 0.0): Double {
