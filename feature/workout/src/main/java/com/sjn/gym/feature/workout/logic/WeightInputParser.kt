...
sealed interface WeightParsingResult {
    data class Success(val weights: List<Double>) : WeightParsingResult

    data class Failure(val message: String) : WeightParsingResult
}

class WeightInputParser @Inject constructor() {
    fun parse(input: String, equipment: String): WeightParsingResult {
        ...
        if (!isStackable) {
            ...
            if (normalizedInput.contains(" ") || normalizedInput.contains("x", ignoreCase = true)) {
                return WeightParsingResult.Failure(
                    "Stacking weights is not allowed for $equipment."
                )
            }
